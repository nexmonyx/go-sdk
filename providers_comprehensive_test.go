...
// TestProvidersService_ConcurrentOperations tests concurrent operations on providers
func TestProvidersService_ConcurrentOperations(t *testing.T) {
	tests := []struct {
		name              string
			...
			for i := 0; i < tt.concurrencyLevel; i++ {
				...
				go func(id int) {
					...
					for j := 0; j < tt.operationsPerGoro; j++ {
						...
						switch tt.operation {
						case "list":
							_, _, apiErr = client.Providers.List(context.Background(), nil)
						case "create":
							req := &ProviderCreateRequest{
								Name:         "Test Provider",
								ProviderType: "aws",
							}
							_, _, apiErr = client.Providers.Create(context.Background(), req)
						}
