...
// TestServersService_ConcurrentOperations tests concurrent operations on servers
func TestServersService_ConcurrentOperations(t *testing.T) {
	tests := []struct {
		name              string
		...
		{
			name:              "concurrent Register - medium concurrency",
			concurrencyLevel:  30,
			operationsPerGoro: 2,
			operation:         "register",
			mockStatus:        http.StatusCreated,
			...
			for i := 0; i < tt.concurrencyLevel; i++ {
				...
				go func(id int) {
					...
					for j := 0; j < tt.operationsPerGoro; j++ {
						...
						case "get":
							_, apiErr = client.Servers.Get(context.Background(), "srv-1")
						case "register":
							_, apiErr = client.Servers.Register(context.Background(), "test-server", 1)
						case "update":
							serverObj := &Server{Hostname: "updated"}
							_, apiErr = client.Servers.Update(context.Background(), "srv-1", serverObj)
						}
